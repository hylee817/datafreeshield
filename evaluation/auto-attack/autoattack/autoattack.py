...
class AutoAttack():
    def __init__(self, model, norm='Linf', eps=.3, seed=None, verbose=True,
        ...
        if not self.is_tf_model:
            ...
            self.fab = FABAttack_PT(self.model, n_restarts=5, n_iter=100, eps=self.epsilon, seed=self.seed,
                norm=self.norm, verbose=False, device=self.device, n_target_classes=n_classes-1)
            ...
            self.apgd_targeted = APGDAttack_targeted(self.model, n_restarts=1, n_iter=100, verbose=False,
                ...
                logger=self.logger,  n_target_classes=n_classes-1)
        ...
        if version in ['standard', 'plus', 'rand']:
            self.set_version(version, n_classes)
    ...
    def run_standard_evaluation(self,
        ...
        with torch.no_grad():
            ...
            startt = time.time()
            for attack in tqdm(attacks_to_run):
        ...
        if return_labels:
            if return_accuracy:
                return x_adv, y_adv, robust_accuracy_dict, robust_accuracy
            else:
                return x_adv, y_adv
        else:
            if return_accuracy:
    ...
    def set_version(self, version='standard',n_classes=9):
        if self.verbose:
        ...
        if version == 'standard':
            ...
            if self.norm in ['Linf', 'L2']:
                ...
                self.apgd_targeted.n_target_classes = n_classes - 1
            ...
            # self.apgd_targeted.n_target_classes = 9
            self.fab.n_target_classes = n_classes - 1
            self.apgd_targeted.n_target_classes = n_classes - 1
